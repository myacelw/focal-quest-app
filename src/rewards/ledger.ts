 ...
 *
 * 涨价两次的由来：
 * ① 50 → 500：50 分不到一天所得的零头，等于白送，真正拦住补签的只有次数上限。
 * ② 500 → 2000（2026-07-29 按云端真实数据重定）：当初那句"一天保底 180 分、
 *    稳定期约 360 分"是拿**完成门槛**（每分钟 5 个 × 6 分钟 = 30 个答对）当典型值估的，
 *    而实测**日均 1635 分**——500 只值 0.31 天，"≈ 练一天所得"这个设计意图从没成立过。
 *
 * 与 `PACK_COST` 同价是刻意的：两个花销口同价，孩子面对的是"补一天签 vs 开一包卡"
 * 这个干净的取舍，而不是两套不可比的价签。
 *
 * **要调价先重算日均进账**（checkin 的 dailyPoints 在真实训练日的平均），别照抄估计值。
 */
export const REPAIR_COST = 2000
