...
**Goal:** 堵住「挂机 3 分钟照样打卡拿分续连续天数」这个漏洞——一轮训练结束时按时长比例要求最低答对数（每分钟 5 个 / 整次双眼合计），不达标就不打卡、不发积分、连续天数不续，结算页明确告知差多少并给「再练一轮」按钮。

**Architecture:** 新增 `src/training/goal.ts`（门槛的全部常量与纯函数 + 唯一的时长读取口 `readDurationSec`），闸门本身放进 `src/data/checkin.ts` 的 `doCheckIn()`——那是唯一写 `checkins` 表的正规路径，放 UI 层会被绕过。`CheckinResult` 从"两态布尔"改成三态 `outcome: 'checked-in' | 'already' | 'below-goal'`，`TrainingPage` 按 `outcome` 分流出一个新的不达标结算页并提供 `restartRound()` 就地复位（**刻意不用 `key` 重挂载**——那会 stop 掉 vosk 再重启，正是 commit `89743b6` 修过的换眼白屏内存尖峰路径）。配套堵住补签卡这条付费绕过通道（`canRepair` 新增 `'attempted'` 闸门）。
...
- [ ] **Step 0.3: 读 spec 一遍。** `docs/superpowers/specs/2026-07-27-训练完成门槛-design.md`。特别是 **§3.5（门槛为什么必须按"真实练过的时长"算 —— 评审 blocker）**、§4（为什么是整次判定而不是单眼各判）、§4.3 末（发分不对称与那个封顶阀门）、§5.4（勋章为什么照常解锁）、**§5.6（补签闸门为什么不能用"那天有没有 session 行"当判据）**、§8（重来为什么不用 `key` 重挂载 / §8.5 跨零点）。
...
- [ ] **Step 0.6: 两个产品决策点——先问用户，拿到明确答复再动 Task 2。**
  两条都是**货币/行为语义**，不能埋在实现里默默定了（spec §4.3、§3.5 已写清）：
  ...
  2. **长期把「单眼时长」钉在 1 分钟**（每天只需 10 个答对）。本计划**不修**，因为它不是本次引入的——今天在完全没有门槛的情况下改档就已经把训练量砍到 1/3。若用户要求一并堵，作为**本计划之外**的独立改动：把时长卡片挪进 `<Collapsible>`（`src/settings/Collapsible.tsx`）或给设置页加家长口令。
     （本计划已修的是另一半、也是真漏洞：**事后改档不能追溯降低当天门槛**，见 Task 1 的 `goalForDay`。）
...
| 路径 | 职责 |
|---|---|
| `src/training/goal.ts` | 门槛的**全部**常量 + 纯函数 `sanitizeDurationSec` / `goalPerEye` / `goalPerRound` / **`goalForDay`** / **`goalForPastDay`** / **`dayFellShort`** / `shortfall` / `meetsGoal` / **`pointsCorrect`** + 薄包装 `readDurationSec` |
| `src/training/goal.test.ts` | 上述纯函数单测（含脏 `durationSec` 钳制、"事后改档不降门槛"、`dayFellShort` 三态、封顶开关） |
| `src/data/checkin.test.ts` | `doCheckIn` 三态的 IndexedDB 集成单测（本仓 `doCheckIn` 此前**零单测**），含"练完再调小档位不降门槛"与发分不对称的数值锚定 |
