...
⚠️ 这一处是本任务的**真正理由**。原来的三元链要为每个新世界加一段，而它有 `: []` 兜底——漏加不报错，只会让新皮肤永远吃不到储备怪。改成按 id 索引后这条路不存在了，前提是「皮肤 id 与世界 id 同名」，那条由 Task 5 的一条测试锚住。
...
**Interfaces:**
- Consumes: Task 3 定下的 48 个 slug（见该任务的三张表，文件名必须逐字对应）
- Produces: 无代码接口
...
| 图 | 世界 | 画风 | 内容 |
|---|---|---|---|
| 1 | forest | 柔和插画（同卡册） | Task 3 表一的 16 个 slug |
| 2 | space | 半写实科幻游戏美术 | Task 3 表二的 16 个 slug |
| 3 | shrine | 16-bit 像素风 | Task 3 表三的 16 个 slug |
...
Run（把 `<sheet>` 换成实际文件名，`<slug>` 按 Task 3 的表逐格对应）：
...
Run: `npm test`
Expected: PASS，比 Task 1 结束时多 7 条。
...
**Interfaces:**
- Consumes: `reserveMonstersOfWorld('forest')`（Task 3）、`StageProps`（`src/skins/types.ts`）、`TumblingE`、`asset`、`useT`
- Produces:
...
Run: `npm test`
Expected: PASS，比 Task 3 结束时多 10 条。
...
**Interfaces:**
- Consumes: `ForestStage`（Task 4）、`WORLDS` / `isWorld`（Task 1）
...
Run: `npm test`
Expected: PASS，比 Task 4 结束时多 **4** 条：registry 新增「森林 4000 分档」与「皮肤 id 必须有同名世界」两条、i18n 新增两条。（`has plain, space, shrine and forest` 那条是**改**既有断言，不计入新增。）
...
- [ ] `npm test` 全绿，总数 = 基线 + **28**（Task 1 加 7、Task 3 加 7、Task 4 加 10、Task 5 加 4）
