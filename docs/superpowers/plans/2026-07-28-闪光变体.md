...
- [ ] `npm test` 全绿，总数 = 基线 + **21**（Task 1 加 6、Task 2 加 7、Task 3 加 4、Task 4 加 3、Task 5 加 1；Task 2 另有 16 条既有用例被改写而非新增）。基线 697 → 终点 **718 个 / 74 文件**
- [ ] `npm run typecheck` 与 `npm run typecheck:functions` 都无输出
- [ ] `npm run build` 成功
- [ ] **同步层零改动**——确认 `src/sync/` 下一个文件都没被碰过（`git diff --stat` 里不该出现 `src/sync/`）。闪光靠独立行天然走通既有的 `monster` kind
- [ ] 按 [CLAUDE.md](CLAUDE.md) 的惯例把本迭代写进「当前状态」，含「闪光是独立行不是行内字段」与「`SHINY_CHANCE` 只控惊喜密度不控总时长」两条口径
