...
**不新建文件。** 本计划是在既有图鉴模块上加一个维度，没有值得独立成文件的新职责。
...
Run: `npm test`
Expected: PASS，条数与 Task 3 结束时一致（本任务不加测试——都是组件视觉，`MonsterImage` 与揭示卡在 node 环境下没有渲染基建可验，验收放在 Task 5 的浏览器检查里）。
...
```bash
git add src/dex/MonsterImage.tsx src/training/TrainingPage.tsx
...
- [ ] `npm test` 全绿，总数 = 基线 + **14**（Task 1 加 6、Task 2 加 7、Task 3 加 4、Task 5 加 1；Task 2 另有 16 条既有用例被改写而非新增）
