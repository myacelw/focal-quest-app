...
## 实现记录（2026-07-25，Task 1-5 已完成）

实际结果与计划的差异，供 Task 6 与后续迭代参考：

1. **测试基线**：计划按 197 写的预期数字全部偏低 3——3a 的 `sw-route.test.ts` 已把基线推到 **200**。最终全量为 **241 passed / 31 files**（200 基线 + crypto 15 + invite 8 + ratelimit 4+3 + sync-validate 11）。
2. **注册限速改为两道额度**（`REGISTER_OK_PER_IP_MAX=20`/日 + `REGISTER_FAIL_PER_IP_MAX=10`/小时），取代计划里的单一 `REGISTER_PER_IP_MAX=5`。原因见 spec §9.2 修订：单一额度会让抄错邀请码的家长被锁一整天。另加 3 个测试锚定该意图。
3. **引导码默认值改为 `TESTBEEF`**：计划里写的 `TESTBOOT` 含字母 **O**，而邀请码字符表刻意剔除了 O/0/I/1/L，形状校验先于引导码比对，会直接 400。
4. **集成脚本不做自动清理限速**：计划曾设想脚本自清 `rl.*` 计数，但 `wrangler pages dev` 持有本地 D1 文件锁，运行期间清不掉（必然失败）。改为放宽额度使其极少发生 + 提供 `npm run db:reset:limits` 应急（须先停服务）。**连跑三遍均 31/31 通过**，可重复性已验证。
5. **`process.exitCode` 而非 `process.exit()`**：Windows + Node 24 下带未关闭 fetch keep-alive 连接时 `process.exit()` 会触发 libuv 断言、退出码变 127（3a 的 `check-coi.mjs` 已踩过同一坑）。
6. Task 1 的 `typecheck:functions` 在当时无 `.ts` 文件会报 "No inputs were found"，属正常，Task 2 起可用。
