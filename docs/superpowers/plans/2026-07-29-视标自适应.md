...
function OptotypeAdjustCard({ decision, dateStr }: { decision: AdjustDecision; dateStr: string }) {
  ...
  const titleKey = isRevert ? 'optoAuto.reverted' : done ? 'optoAuto.tightened' : 'optoAuto.suggest'
  const hintKey = isRevert ? 'optoAuto.revertedHint' : done ? 'optoAuto.tightenedHint' : 'optoAuto.suggestHint'
