...
- 界面美化·门面 ✅ 完成并合并：从零建立全局视觉系统（src/index.css，风格 B「糖果能量」——紫 #6c4bf0/珊瑚/柠檬/薄荷 + 圆角卡片 + 糖果渐变，CSS 变量 + fq-card/fq-cta/fq-btn/fq-chip/fq-bar/fq-nav 基础类）；导航图标胶囊化、首页珊瑚渐变打卡卡 + 紫 CTA。此前全 app 是浏览器默认宋体白底灰按钮。**风格 B 由闺女在三方向 mockup 中选定，后续逐页统一都遵循它**（114 单测绿）。
- 下一步：界面美化逐页统一（统计/勋章墙/训练准备/结算庆祝页，套 fq-* 类与配色变量）；先 iPad 实测已积累成果（视标 0.7mm / vosk 识别率 / 皮肤观感 / 积分解锁 / 每题换怪兽 / 美化观感）；后端部署（国内轻量服务器）；或迭代 2 其余（限时挑战需确认节奏 / 深海动物需先验 vosk）；或给勋章+皮肤出正式图。
