...
- 迭代 1a（核心训练循环）✅ 完成并合并：Tumbling E 视标、语音+触控答题、翻拍过渡、计时换眼、单节结算（41 单测绿）。
- **已知遗留（下次优先）**：训练视标默认尺寸偏小——按视力阈值算（0.8≈0.7mm，iPad 上仅几像素）。需改成"按实际训练放大"（加放大系数，而非直接用视力阈值），待用户实测反馈孩子看着舒服的尺寸再定。
- 下一步：迭代 1b（每日打卡 + 连续天数 + 积分 + IndexedDB 持久化）。
