...
## 开发命令
> 单测基线：**622 个 / 63 个文件**（2026-07-27 实测 `npm test`，含训练完成门槛新增 60 个 / 4 个文件：`goal` 23 / `checkin` 15 / `goal-gate` 8 / `settings-fold` 7 + `i18n` 2 + `ledger` 5，即门槛前是 562 个 / 59 个文件）。上面各条目里的"NN 单测绿"是当时的历史快照，别拿来当现值；这行也只是当天快照，开工前自己跑一遍 `npm test` 记真实基线。
