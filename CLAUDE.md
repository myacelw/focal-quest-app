...
- 迭代 2·积分解锁皮肤 ✅ 完成并合并：累计分达门槛永久解锁（门槛派生、不扣分、零持久化），太空/神庙各 300 分、朴素免费；准备页锁态按钮 🔒+"再练 N 分"提示+生效皮肤兜底回退（未解锁→plain）；改 `SKIN_UNLOCK_COST` 一处即可调价/全开（106 单测绿）。**注意：这把原本全开的皮肤改成需解锁，属产品行为变更，待父女确认体验；不满意把价都设 0 即回退全开。**
- 下一步（多需父女真机实测才好定方向）：先 iPad 实测已积累成果（视标 0.7mm / vosk 孩子识别率 / 太空vs神庙皮肤观感 / 积分解锁体验）；或迭代 2 其余（限时挑战需确认节奏 / 深海动物识别需先验 vosk 词表）；或给勋章+皮肤出正式图。
